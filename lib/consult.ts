/* ============================================================================
   CONSULT FUNNEL — server layer (RHRLI)

   Owns every call into GoHighLevel for the /c/consult booking page, plus the
   phone-verification codec.

   Why verification is stateless: this app runs on Coolify with no database and
...
export const LOCATION_ID = '8GEloxZleCEfdjQE1I2x'
export const TIMEZONE = 'America/New_York'

/* Verified live 2026-08-14 by reading /calendars/ with the location PIT. These
   already existed in RHRLI's sub-account; the format step selects between them. */
export const CALENDARS = {
  'in-person': 'OSdWDYgLi0pb1wiJ5l1a', // "Consultation: In Person"
  virtual: 'mCnpwSw6zQTufQvVCPij',     // "Consultation: Virtual"
} as const

export type ConsultFormat = keyof typeof CALENDARS
...
/* Attribution and intake fields.
   ...
   UNVERIFIED as of 2026-08-14: the location PIT returns 401 "not authorized for
   this scope" on GET /locations/{id}/customFields, so this map cannot be seeded
   from the live system. A PUT with a guessed key returns 200 succeeded, which
   proves nothing about whether the value bound to a real field or was accepted
   and dropped. Do not treat this as working until the PIT gains the read scope
   and CONSULT_FIELD_MAP is regenerated from the API. */
export const CONSULT_FIELD_MAP: Record<string, string> = {
  // payload key   ->  GHL fieldKey   (SEED FROM THE API, DO NOT TYPE FROM A DOC)
}
...
export async function writeCustomFields(contactId: string, lead: Lead) {
  ...
  if (!entries.length) {
    /* Says which, rather than reporting silence as success. */
