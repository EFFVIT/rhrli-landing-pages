...
export default function ConsultFunnel() {
  ...
  const loadSlots = useCallback(async (format: string) => {
    ...
    try {
      ...
      setSlotsError(`We could not load times just now. Call ${PHONE_DISPLAY} and we will book it for you.`)
    }
  }, [])
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      setVErr(`We could not send the code. Check the number, or call ${PHONE_DISPLAY} and we will book it for you.`)
    ...
    try {
      const r = await fetch('/api/consult/book', {
        ...
        body: JSON.stringify({
          ...
          consent_text_version: '2026-08-14-v1',
        }),
      })
      const d = await r.json()
        ...
        <>
          ...
          {STEPS[step].id === 'about' && (
            <div>
              ...
              <p className="step-note">Both change how the hour is scheduled, nothing else.</p>
              <div className="stack">
              ...
              <p className="step-hint">A previous procedure means the hour is booked longer, because the donor area is assessed as well.</p>
            </div>
          )}
          ...
          {STEPS[step].id === 'time' && (
            <div>
              ...
              {slots && slots.length === 0 && (
                <p className="step-hint">No times are published for the next two weeks. Call {PHONE_DISPLAY} and we will find one.</p>
              )}
              {slots && slots.length > 0 && (
                <div className="slots">
